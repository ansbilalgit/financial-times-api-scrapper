...
using static ft_crawler.Infrastructure.ExcelStructs;
...
namespace ft_crawler.Utilities
{
    public class ExcelUtility
    {
        ...
        public static void ExportToExcelOleDb(DataTable dataTable, string tableName, string outputFilePath,
                                                           bool deleteExistFile)
        {
            // Support for existing file overwrite.
            if (deleteExistFile && File.Exists(outputFilePath))
                File.Delete(outputFilePath);
            ExportToExcelOleDb(dataTable, tableName, outputFilePath);
        }

        private static bool ExportToExcelOleDb(DataTable dataTable, string tableName, string outputFilePath)
        {
            try
            {
                string connectionString = GetConnectionString(outputFilePath);
                // Check for null set.
        ...
        private static string BuildCreateTableCommand(DataTable dataTable, string tabeName)
        {
            ...
            foreach (DataColumn col in dataTable.Columns)
            {
                ...
                sb.AppendFormat("[{0}] {1},", col.Caption.Replace(' ', ' '), type);
            }
        ...
        private static string BuildInsertCommand(DataTable dataTable, string tableName, int rowIndex)
        {
            ...
            foreach (DataColumn col in dataTable.Columns)
                sb.AppendFormat("[{0}],", col.Caption.Replace(' ', ' '));
            sb = sb.Replace(',', ')', sb.ToString().LastIndexOf(','), 1);
